    ...
    private static Dictionary<string, ApiComponentInfo> Build()
    {
        ...
        c[@"FlareDataGrid"] = new ApiComponentInfo(
            ...
            new ApiParameterInfo[]
            {
                ...
                new ApiParameterInfo(@"InfiniteScroll", @"bool", null, @"Infinite scrolling: with an ItemsProvider, rows accumulate page by page as the user scrolls to the bottom, and no pager is shown - this is a paging strategy of its own, so it replaces PageSize's. The provider is called with successive Page values; loading stops when a page returns fewer than PageSize rows or the reported total is reached. Combine with Virtual to recycle the rows that have accumulated - otherwise a long enough scroll ends with the whole set in the DOM.", null, false, false, false, @"FlareDataGrid"),
