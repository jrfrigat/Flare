    ...
    private static Dictionary<string, ApiComponentInfo> Build()
    {
        ...
        c[@"FlareDataGrid"] = new ApiComponentInfo(
            ...
            new ApiParameterInfo[]
            {
                ...
                new ApiParameterInfo(@"PageSize", @"int", null, @"Number of rows on a page, or 0 (the default) for no paging at all: every row goes into one page and the grid scrolls inside whatever height it was given. Paging and scrolling are independent - a paged grid still scrolls when its page is taller than Height. With an ItemsProvider or Queryable, 0 means one request for the whole set. That is what ""no paging"" can mean against a remote source, so set a page size, Virtual or InfiniteScroll when the source is large.", null, false, false, false, @"FlareDataGrid"),
                ...
                new ApiParameterInfo(@"Virtual", @"bool", null, @"Recycles rows: only the visible window plus an overscan margin lives in the DOM (consistent with FlareList.Virtual). Combine with an ItemsProvider to load rows on demand as the user scrolls. Replaces paging for now - PageSize is ignored and no pager is shown - which is the one place where virtualization still changes more than how many rows are rendered.", null, false, false, false, @"FlareDataGrid"),
