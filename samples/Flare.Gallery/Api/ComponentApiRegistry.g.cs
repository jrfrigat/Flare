    ...
    private static Dictionary<string, ApiComponentInfo> Build()
    {
        ...
        c[@"FlareDataGrid"] = new ApiComponentInfo(
            ...
            new ApiParameterInfo[]
            {
                ...
                new ApiParameterInfo(@"Height", @"string", null, @"How tall the scrolling table container may grow, as a CSS length. It is a CAP (max-height), not a height: a grid with fewer rows than that is as tall as its rows. Applies in every mode that scrolls instead of paging - Scroll, Virtual, InfiniteScroll - and is ignored when FillHeight is set. Use an ABSOLUTE length. A percentage resolves against the grid's own box, which is sized by its content unless something above it says otherwise, and a percentage max-height against a content-sized box computes to none - so the cap silently does not exist, the container never scrolls, and a virtual grid is handed a scroller that cannot scroll. When the height is the layout's answer rather than a number you can write down, that is what FillHeight is for.", null, false, false, false, @"FlareDataGrid"),
                ...
                new ApiParameterInfo(@"Scroll", @"bool", null, @"Renders every row inside the grid's own scroll box instead of paging: no pager, a sticky header, and one scrollbar on the table container. The middle ground between pagination and Virtual, and the right choice from a few hundred to a few thousand rows - every row is in the DOM, so browser find, keyboard navigation and printing reach all of them, which row recycling cannot offer. Past that, prefer Virtual. Bound by Height unless FillHeight takes over. With an ItemsProvider or Queryable the whole set is requested in one call, so use Virtual or InfiniteScroll when the source is large and remote. Ignored while Virtual or InfiniteScroll is on.", null, false, false, false, @"FlareDataGrid"),
