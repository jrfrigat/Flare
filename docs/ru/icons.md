...
`Flare.Components` поставляет собственный SVG-набор без внешних зависимостей (`FlareIcons`, ~90 иконок),
которым рисуется дефолтный хром компонентов (шевроны, close, сортировка, тогглы дерева, ...). Работает из
коробки, без дополнительных пакетов.
...
- `FlareIcons.All` перечисляет встроенные id (snake_case).
- Голая строка резолвится во встроенный набор: `<FlareIconView Name="home" />`. Id, которого **нет** во
  встроенном наборе, резолвится в пустую иконку - ядро не откатывается на сторонний шрифт. Предпочитайте
  типизированный член; строковый вариант - это удобство, от которого в будущем релизе откажемся.
