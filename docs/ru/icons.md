...
Две важные вещи. Это переход МЕЖДУ двумя иконками, а не интерполяция одного контура в другой - пару выбирают
в рантайме, поэтому общей структуры путей быть не может. И работает он у `FlareIconView`; компоненты,
которые принимают `FlareIcon` и рендерят его сами, это не затрагивает.
