...
| режим | что делает |
| :-- | :-- |
| `None` (по умолчанию) | мгновенная замена; вью рендерит только сам элемент иконки, без обертки |
| `Fade` | перекрестное затухание на месте |
...
Параметра `Morph` тут нет - переход несет сам тип иконки, и `FlareIconView` не трогает такую иконку даже
