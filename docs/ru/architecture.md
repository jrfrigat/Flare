...
- `StyleFamilyId` называет семейство, чьи стили оформляют тему, и выводится вторым классом на корне
  рядом с собственным классом темы. По умолчанию равен `Id`: тема со своими стилями сама себе
  семейство. Тема, собранная через `Derive`, сохраняет семейство базовой: она лишь переопределяет
  токены и наследует CSS базовой темы, а тот ограничен классом базовой темы. Без этого производная
  тема осталась бы вовсе без стилей.
...
ITheme
  ...
  +-- StyleFamilyId (string)                    - чьи стили оформляют тему (по умолчанию Id)
