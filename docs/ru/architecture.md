...
- `ScriptAssets` перечисляет JS-модули, которыми владеет тема; они грузятся один раз, когда тема
  становится активной, в том числе когда она активна только для поддерева `FlareThemeScope`. Список
  необязательный и у всех встроенных тем пустой: собственная отрисовка темы принадлежит ее стилям
  везде, где CSS способен ее выразить. Модуль обязан опираться на свои публичные CSS-классы, а не на
  id темы, иначе он не будет обслуживать производные от нее темы.
- `Base` называет тему, поверх которой построена эта (null у темы, которая стоит сама по себе).
  Стили каждой темы ограничены ее собственным классом, поэтому корень несет по классу
  `flare-theme-{id}` на каждое поколение цепочки (`ThemeLineage.RootClasses`, считается один раз на
  экземпляр темы), и CSS каждого предка продолжает действовать. `Derive` и
  `FlareThemeBuilder.WithBase` ставят его и подключают ассеты предков раньше собственных, поэтому
  правило потомка выигрывает при равенстве за счет порядка подключения.
...
ITheme
  ...
  +-- ScriptAssets (IReadOnlyList<string>)      - JS-модули темы (необязательны, по умолчанию пусто)
  +-- Base (ITheme?)                            - тема, поверх которой построена эта (null - сама по себе)
