...
> [English version ->](../en/ai-agents.md) - [Начало работы](getting-started.md) - [API Reference](../../api/)
...
параметров любого компонента смотрите в сгенерированном [API Reference](../../api/).

---

## 1. Что такое Flare

- UI-библиотека для **.NET 10 Blazor**. Работает и в Blazor WebAssembly, и в Blazor Server.
...
```sh
# Ядро (тянет Flare.Core + Flare.Components, 100+ компонентов):
...
{
    opts.DefaultTheme   = new Md3Theme();        // активная дизайн-система
...
Реальные имена всех компонентов (параметры каждого - в [API Reference](../../api/)):
...
- **[API Reference](../../api/)** - каждый public-тип, параметр и enum (сгенерировано из XML-доков).
