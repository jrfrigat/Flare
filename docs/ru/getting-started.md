...
- **`Height` ограничивает ВЕСЬ компонент и действует в любом режиме.** Тулбар, пагинатор и футер
  ...
  по-прежнему нужен предок с определённой высотой - то же самое `FillHeight` говорит без числа.
...
отведённую высоту, - с пагинацией или без.

| Параметр | Что решает | По умолчанию |
| :-- | :-- | :-- |
| `Height` / `FillHeight` | какой высоты компонент, а значит - прокручивается ли он | `400px` |
| `PageSize` | `0` кладёт все строки на одну страницу; положительный размер добавляет пагинатор | `0` |
| `StickyHeader` | остаётся ли шапка на месте, пока строки едут под ней | `true` |
| `Virtual` | в DOM живут только видимые строки; пагинацию по-прежнему заменяет | выкл. |
