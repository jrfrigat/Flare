...
- **Бокс, который ДАЁТ высоту, `FillHeight` не берёт.** Заполнение заменяет собственную высоту
  элемента долей родительской, поэтому если задать и то и другое на одном боксе, проиграет как раз
  написанная вами высота.
