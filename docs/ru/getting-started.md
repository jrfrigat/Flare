...
- `Flare.Components` - 160+ UI-компонентов
...
дважды. Он по-прежнему дожидается стилей активной темы и `StyleAsset` активной палитры, узнает вашу
ссылку по адресу, как бы он ни был записан (`_content/...`, `/_content/...`, `./_content/...`), и
добавляет только недостающее - тему или палитру, на которую пользователь переключился во время работы,
или файл, который появился в новой версии темы. Забытая ссылка обойдется поздним запросом, а не
неоформленной страницей.
