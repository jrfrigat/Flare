...
Цепочка держится на определённых высотах. `FlareLayout` - это `100dvh`, его область контента - строка
`1fr` этой сетки. Не хватало рамки вокруг страницы: своей высоты у неё не было, поэтому любой процент
ниже разрешался в `auto`. `FillHeight` её даёт, и это одно и то же слово на всех уровнях - способ
...
<FlareLayoutContent FillHeight="true">
    <FlareTabs FillHeight="true">
        <FlareTab Label="Заказы">
            @* Scroll: все строки, без пагинатора, с прилипшей шапкой.
...
- **`FillHeight` нужен предок с высотой.** Странице её даёт `FlareLayoutContent FillHeight`. В любом
  другом месте - в демо, в карточке - высоту нужно задать (`Style="height:24rem"`).
- **Нужен каждому звену цепочки.** Таблице внутри панели вкладок нужен `FillHeight` и на таблице, и на
  наборе вкладок, *и* на области контента; одно пропущенное звено разрешается в `auto`, и вся цепочка
  схлопывается до высоты содержимого.
...
- **`Height` принимает абсолютную длину, и это ограничение сверху.** `Height="50%"` выглядит разумно и
  не делает ничего: это процентный `max-height` против блока, размер которого задаёт его собственное
  содержимое, а такой процент CSS вычисляет в `none`. Ограничения нет, грид не прокручивается, а в
  режиме `Virtual` он получает контейнер, который прокрутиться не может. Процентная высота - это
  работа `FillHeight`.

Три режима строк `FlareDataGrid` отвечают на три разных вопроса:

| Режим | Что рисует | Когда |
| :-- | :-- | :-- |
| пагинация (по умолчанию) | одну страницу и пагинатор | пользователь ходит по страницам |
| `Scroll="true"` | все строки, без пагинатора, прилипшая шапка | несколько сотен - несколько тысяч строк; поиск браузера и печать должны доставать до всех |
| `Virtual="true"` | только видимые строки | десятки тысяч строк |
