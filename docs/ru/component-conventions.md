...
Состояние, в котором элемент **покоится** (`--open`, `--visible`, `--shown`, `--expanded`), задаёт
...
select внутри неё рисовался на 896px правее окна и читался как «список открылся пустым».
...
не эмитит, и он не несёт дизайнерского решения. А фолбэк, называющий ЦВЕТ, решает за все темы сразу, как
компонент выглядит. Правило сформулировано именно так, чтобы не понадобился список исключений: 44 чтения
по 19 токенам проходят без перечисления, и новое пройдёт тоже, пока оно identity. Сторож -
...
прогонять всю краску библиотеки через один канал стоило бы дороже, чем даёт. Отдельный слой заводится
...
- **`_last*` - зеркало параметра, а не состояния.** Оно отвечает ровно на один вопрос: изменился ли
  ...
  (отчёт: "работал ненадёжно без внешнего управления состоянием").
...
- Если текст/разметка вызывающего - это **всё содержимое** компонента, слот называется `ChildContent`
...
Гвард: `SlotNameTests`. В нём же записаны исключения - `Columns` и `Grouping` у DataGrid остаются, потому
