...
не ответит на неё. Она описывает, что происходит при СМЕНЕ иконки (`FlareIconView.Morph` и
`FlareMorphIcon`), и не делает ничего, пока приложение не попросит переход, - так что тема без мнения
на этот счёт может её припарковать и идти дальше.
...
**Ловушка - это `MorphEasing`: он задаёт тайминг только ДВИЖЕНИЯ, но не затухания.** Разделение
существует потому, что сюда обычно ставят пружину, а пружина проскакивает цель: сглаженная доля больше
1 прогоняет прозрачность за её конечную точку, затухание заканчивается примерно на трети пути, и
