...
- **`FlareFloatingActionButton`** is distinct. Aligned now on `OnClick`/`FocusAsync`. No `Href`/`Loading`
  by design (a FAB is a screen-level action, not a link; deliberate non-goal).
