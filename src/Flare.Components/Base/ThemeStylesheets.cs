...
public enum ThemeStylesheets
{
    ...
    /// provider emits no links of its own; it still waits for the active theme's sheets, reuses a link
    /// the page already has for the same address, and adds only a sheet that is missing, such as one for
    /// a theme the user switches to at run time.
    /// </summary>
    Manual,
}
