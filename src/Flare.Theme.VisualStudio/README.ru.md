...
Тема в духе Visual Studio (2022/2026), светлая и тёмная, для библиотеки компонентов
...
её ОБОЛОЧКИ - то, на что вы показали бы пальцем в IDE, - с компонентами, которые их воспроизводят.
...
| Оболочка целиком | `FlareIdeLayout` | собирает всё перечисленное в один каркас |

Всё остальное - кнопки, поля, флажки, таблицы - это обычный набор компонентов Flare в токенах этой
...
- **Выделение - плоская заливка, а не тональная плёнка**: та же модель дискретных состояний, что и у
  Fluent, поэтому тема задаёт свои токены слоя состояния, а не полагается на значение из Material.
