namespace Flare.Css.Tokens;

/// <summary>CSS variable tokens for tooltip popup.</summary>
public static class TooltipPopup
{
    private const string Prefix = $"{Vars.Flare}-tooltip";

    /// <summary>CSS custom-property name for the surface color token.</summary>
    public const string SurfaceColor = $"{Prefix}-surface-color";
    /// <summary>CSS custom-property name for the text color token.</summary>
    public const string TextColor = $"{Prefix}-text-color";
    /// <summary>CSS custom-property name for the radius token.</summary>
    public const string Radius = $"{Prefix}-radius";
    /// <summary>CSS custom-property name for the padding token.</summary>
    public const string Padding = $"{Prefix}-padding";
    /// <summary>CSS custom-property name for the max width token.</summary>
    public const string MaxWidth = $"{Prefix}-max-width";
    /// <summary>CSS custom-property name for the font family token.</summary>
    public const string FontFamily = $"{Prefix}-font-family";
    /// <summary>CSS custom-property name for the font size token.</summary>
    public const string FontSize = $"{Prefix}-font-size";
    /// <summary>CSS custom-property name for the font weight token.</summary>
    public const string FontWeight = $"{Prefix}-font-weight";
    /// <summary>CSS custom-property name for the line height token.</summary>
    public const string LineHeight = $"{Prefix}-line-height";
    /// <summary>CSS custom-property name for the offset token.</summary>
    public const string Offset = $"{Prefix}-offset";
    /// <summary>CSS custom-property name for the arrow size token.</summary>
    public const string ArrowSize = $"{Prefix}-arrow-size";
    /// <summary>CSS custom-property name for the transition duration token.</summary>
    public const string TransitionDuration = $"{Prefix}-transition-duration";
    /// <summary>CSS custom-property name for the transition easing token.</summary>
    public const string TransitionEasing = $"{Prefix}-transition-easing";
    /// <summary>CSS custom-property name for the show delay token.</summary>
    public const string ShowDelay = $"{Prefix}-show-delay";
    /// <summary>CSS custom-property name for the hide delay token.</summary>
    public const string HideDelay = $"{Prefix}-hide-delay";
}

/// <summary>CSS variable tokens for popover popup.</summary>
public static class PopoverPopup
{
    private const string Prefix = $"{Vars.Flare}-popover";

    /// <summary>CSS custom-property name for the surface color token.</summary>
    public const string SurfaceColor = $"{Prefix}-surface-color";
    /// <summary>CSS custom-property name for the radius token.</summary>
    public const string Radius = $"{Prefix}-radius";
    /// <summary>CSS custom-property name for the elevation token.</summary>
    public const string Elevation = $"{Prefix}-elevation";
    /// <summary>CSS custom-property name for the padding token.</summary>
    public const string Padding = $"{Prefix}-padding";
    /// <summary>CSS custom-property name for the min width token.</summary>
    public const string MinWidth = $"{Prefix}-min-width";
    /// <summary>CSS custom-property name for the max width token.</summary>
    public const string MaxWidth = $"{Prefix}-max-width";
    /// <summary>CSS custom-property name for the max height token.</summary>
    public const string MaxHeight = $"{Prefix}-max-height";
    /// <summary>CSS custom-property name for the offset token.</summary>
    public const string Offset = $"{Prefix}-offset";
    /// <summary>CSS custom-property name for the arrow size token.</summary>
    public const string ArrowSize = $"{Prefix}-arrow-size";
    /// <summary>CSS custom-property name for the scrim color token.</summary>
    public const string ScrimColor = $"{Prefix}-scrim-color";
    /// <summary>CSS custom-property name for the transition duration token.</summary>
    public const string TransitionDuration = $"{Prefix}-transition-duration";
    /// <summary>CSS custom-property name for the transition easing token.</summary>
    public const string TransitionEasing = $"{Prefix}-transition-easing";
}

/// <summary>CSS variable tokens for data grid field.</summary>
public static class DataGridField
{
    private const string Prefix = $"{Vars.Flare}-datagrid";

    /// <summary>CSS custom-property name for the surface color token.</summary>
    public const string SurfaceColor = $"{Prefix}-surface-color";
    /// <summary>CSS custom-property name for the header bg token.</summary>
    public const string HeaderBg = $"{Prefix}-header-bg";
    /// <summary>CSS custom-property name for the header color token.</summary>
    public const string HeaderColor = $"{Prefix}-header-color";
    /// <summary>CSS custom-property name for the header font family token.</summary>
    public const string HeaderFontFamily = $"{Prefix}-header-font-family";
    /// <summary>CSS custom-property name for the header font size token.</summary>
    public const string HeaderFontSize = $"{Prefix}-header-font-size";
    /// <summary>CSS custom-property name for the header font weight token.</summary>
    public const string HeaderFontWeight = $"{Prefix}-header-font-weight";
    /// <summary>CSS custom-property name for the header height token.</summary>
    public const string HeaderHeight = $"{Prefix}-header-height";
    /// <summary>CSS custom-property name for the header padding token.</summary>
    public const string HeaderPadding = $"{Prefix}-header-padding";
    /// <summary>CSS custom-property name for the row height token.</summary>
    public const string RowHeight = $"{Prefix}-row-height";
    /// <summary>CSS custom-property name for the row height dense token.</summary>
    public const string RowHeightDense = $"{Prefix}-row-height-dense";
    /// <summary>CSS custom-property name for the cell padding token.</summary>
    public const string CellPadding = $"{Prefix}-cell-padding";
    /// <summary>CSS custom-property name for the cell color token.</summary>
    public const string CellColor = $"{Prefix}-cell-color";
    /// <summary>CSS custom-property name for the cell font family token.</summary>
    public const string CellFontFamily = $"{Prefix}-cell-font-family";
    /// <summary>CSS custom-property name for the cell font size token.</summary>
    public const string CellFontSize = $"{Prefix}-cell-font-size";
    /// <summary>CSS custom-property name for the selected row bg token.</summary>
    public const string SelectedRowBg = $"{Prefix}-selected-row-bg";
    /// <summary>CSS custom-property name for the selected row color token.</summary>
    public const string SelectedRowColor = $"{Prefix}-selected-row-color";
    /// <summary>CSS custom-property name for the hover row bg token.</summary>
    public const string HoverRowBg = $"{Prefix}-hover-row-bg";
    /// <summary>CSS custom-property name for the sort icon color token.</summary>
    public const string SortIconColor = $"{Prefix}-sort-icon-color";
    /// <summary>CSS custom-property name for the sort icon active color token.</summary>
    public const string SortIconActiveColor = $"{Prefix}-sort-icon-active-color";
    /// <summary>CSS custom-property name for the border color token.</summary>
    public const string BorderColor = $"{Prefix}-border-color";
    /// <summary>CSS custom-property name for the border width token.</summary>
    public const string BorderWidth = $"{Prefix}-border-width";
    /// <summary>CSS custom-property name for the filter row bg token.</summary>
    public const string FilterRowBg = $"{Prefix}-filter-row-bg";
    /// <summary>CSS custom-property name for the group header bg token.</summary>
    public const string GroupHeaderBg = $"{Prefix}-group-header-bg";
    /// <summary>CSS custom-property name for the group header color token.</summary>
    public const string GroupHeaderColor = $"{Prefix}-group-header-color";
    /// <summary>CSS custom-property name for the toolbar bg token.</summary>
    public const string ToolbarBg = $"{Prefix}-toolbar-bg";
    /// <summary>CSS custom-property name for the toolbar height token.</summary>
    public const string ToolbarHeight = $"{Prefix}-toolbar-height";
    /// <summary>CSS custom-property name for the toolbar padding token.</summary>
    public const string ToolbarPadding = $"{Prefix}-toolbar-padding";
    /// <summary>CSS custom-property name for the empty state bg token.</summary>
    public const string EmptyStateBg = $"{Prefix}-empty-state-bg";
    /// <summary>CSS custom-property name for the empty state color token.</summary>
    public const string EmptyStateColor = $"{Prefix}-empty-state-color";
    /// <summary>CSS custom-property name for the resize handle width token.</summary>
    public const string ResizeHandleWidth = $"{Prefix}-resize-handle-width";
    /// <summary>CSS custom-property name for the resize handle color token.</summary>
    public const string ResizeHandleColor = $"{Prefix}-resize-handle-color";
    /// <summary>CSS custom-property name for the column picker bg token.</summary>
    public const string ColumnPickerBg = $"{Prefix}-column-picker-bg";
    /// <summary>CSS custom-property name for the column picker elevation token.</summary>
    public const string ColumnPickerElevation = $"{Prefix}-column-picker-elevation";
}

/// <summary>CSS variable tokens for card field.</summary>
public static class CardField
{
    private const string Prefix = $"{Vars.Flare}-card";

    /// <summary>CSS custom-property name for the elevated bg token.</summary>
    public const string ElevatedBg = $"{Prefix}-elevated-bg";
    /// <summary>CSS custom-property name for the filled bg token.</summary>
    public const string FilledBg = $"{Prefix}-filled-bg";
    /// <summary>CSS custom-property name for the filled border token.</summary>
    public const string FilledBorder = $"{Prefix}-filled-border";
    /// <summary>CSS custom-property name for the outlined bg token.</summary>
    public const string OutlinedBg = $"{Prefix}-outlined-bg";
    /// <summary>CSS custom-property name for the outlined border token.</summary>
    public const string OutlinedBorder = $"{Prefix}-outlined-border";
    /// <summary>CSS custom-property name for the tonal bg token.</summary>
    public const string TonalBg = $"{Prefix}-tonal-bg";
    /// <summary>CSS custom-property name for the tonal color token.</summary>
    public const string TonalColor = $"{Prefix}-tonal-color";
    /// <summary>CSS custom-property name for the text color token.</summary>
    public const string TextColor = $"{Prefix}-text-color";
    /// <summary>CSS custom-property name for the radius token.</summary>
    public const string Radius = $"{Prefix}-radius";
    /// <summary>CSS custom-property name for the elevation token.</summary>
    public const string Elevation = $"{Prefix}-elevation";
    /// <summary>CSS custom-property name for the elevation hover token.</summary>
    public const string ElevationHover = $"{Prefix}-elevation-hover";
    /// <summary>CSS custom-property name for the selected border token.</summary>
    public const string SelectedBorder = $"{Prefix}-selected-border";
    /// <summary>CSS custom-property name for the selected bg token.</summary>
    public const string SelectedBg = $"{Prefix}-selected-bg";
    /// <summary>CSS custom-property name for the state layer token.</summary>
    public const string StateLayer = $"{Prefix}-state-layer";
    /// <summary>CSS custom-property name for the padding top token.</summary>
    public const string PaddingTop = $"{Prefix}-padding-top";
    /// <summary>CSS custom-property name for the padding right token.</summary>
    public const string PaddingRight = $"{Prefix}-padding-right";
    /// <summary>CSS custom-property name for the padding bottom token.</summary>
    public const string PaddingBottom = $"{Prefix}-padding-bottom";
    /// <summary>CSS custom-property name for the padding left token.</summary>
    public const string PaddingLeft = $"{Prefix}-padding-left";
    /// <summary>CSS custom-property name for the content padding token.</summary>
    public const string ContentPadding = $"{Prefix}-content-padding";
    /// <summary>CSS custom-property name for the header padding token.</summary>
    public const string HeaderPadding = $"{Prefix}-header-padding";
    /// <summary>CSS custom-property name for the footer padding token.</summary>
    public const string FooterPadding = $"{Prefix}-footer-padding";
    /// <summary>CSS custom-property name for the actions padding token.</summary>
    public const string ActionsPadding = $"{Prefix}-actions-padding";
    /// <summary>CSS custom-property name for the actions gap token.</summary>
    public const string ActionsGap = $"{Prefix}-actions-gap";
    /// <summary>CSS custom-property name for the media radius token.</summary>
    public const string MediaRadius = $"{Prefix}-media-radius";
    /// <summary>CSS custom-property name for the title color token.</summary>
    public const string TitleColor = $"{Prefix}-title-color";
    /// <summary>CSS custom-property name for the title font family token.</summary>
    public const string TitleFontFamily = $"{Prefix}-title-font-family";
    /// <summary>CSS custom-property name for the title font size token.</summary>
    public const string TitleFontSize = $"{Prefix}-title-font-size";
    /// <summary>CSS custom-property name for the subtitle color token.</summary>
    public const string SubtitleColor = $"{Prefix}-subtitle-color";
    /// <summary>CSS custom-property name for the subtitle font family token.</summary>
    public const string SubtitleFontFamily = $"{Prefix}-subtitle-font-family";
    /// <summary>CSS custom-property name for the subtitle font size token.</summary>
    public const string SubtitleFontSize = $"{Prefix}-subtitle-font-size";
    /// <summary>CSS custom-property name for the transition duration token.</summary>
    public const string TransitionDuration = $"{Prefix}-transition-duration";
    /// <summary>CSS custom-property name for the transition easing token.</summary>
    public const string TransitionEasing = $"{Prefix}-transition-easing";
}

/// <summary>CSS variable tokens for avatar field.</summary>
public static class AvatarField
{
    private const string Prefix = $"{Vars.Flare}-avatar";

    /// <summary>CSS custom-property name for the surface color token.</summary>
    public const string SurfaceColor = $"{Prefix}-surface-color";
    /// <summary>CSS custom-property name for the text color token.</summary>
    public const string TextColor = $"{Prefix}-text-color";
    /// <summary>CSS custom-property name for the icon color token.</summary>
    public const string IconColor = $"{Prefix}-icon-color";
    /// <summary>CSS custom-property name for the rounded radius token.</summary>
    public const string RoundedRadius = $"{Prefix}-rounded-radius";
    /// <summary>CSS custom-property name for the square radius token.</summary>
    public const string SquareRadius = $"{Prefix}-square-radius";
    /// <summary>CSS custom-property name for the size xs token.</summary>
    public const string SizeXs = $"{Prefix}-size-xs";
    /// <summary>CSS custom-property name for the size sm token.</summary>
    public const string SizeSm = $"{Prefix}-size-sm";
    /// <summary>CSS custom-property name for the size md token.</summary>
    public const string SizeMd = $"{Prefix}-size-md";
    /// <summary>CSS custom-property name for the size lg token.</summary>
    public const string SizeLg = $"{Prefix}-size-lg";
    /// <summary>CSS custom-property name for the size xl token.</summary>
    public const string SizeXl = $"{Prefix}-size-xl";
    /// <summary>CSS custom-property name for the font family token.</summary>
    public const string FontFamily = $"{Prefix}-font-family";
    /// <summary>CSS custom-property name for the font size token.</summary>
    public const string FontSize = $"{Prefix}-font-size";
    /// <summary>CSS custom-property name for the font weight token.</summary>
    public const string FontWeight = $"{Prefix}-font-weight";
    /// <summary>CSS custom-property name for the group border width token.</summary>
    public const string GroupBorderWidth = $"{Prefix}-group-border-width";
    /// <summary>CSS custom-property name for the group border color token.</summary>
    public const string GroupBorderColor = $"{Prefix}-group-border-color";
    /// <summary>CSS custom-property name for the group overflow bg token.</summary>
    public const string GroupOverflowBg = $"{Prefix}-group-overflow-bg";
    /// <summary>CSS custom-property name for the group overflow color token.</summary>
    public const string GroupOverflowColor = $"{Prefix}-group-overflow-color";
}

/// <summary>CSS variable tokens for progress field.</summary>
public static class ProgressField
{
    private const string Prefix = $"{Vars.Flare}-progress";

    /// <summary>CSS custom-property name for the track color token.</summary>
    public const string TrackColor = $"{Prefix}-track-color";
    /// <summary>CSS custom-property name for the indicator color token.</summary>
    public const string IndicatorColor = $"{Prefix}-indicator-color";
    /// <summary>CSS custom-property name for the circular color token.</summary>
    public const string CircularColor = $"{Prefix}-circular-color";
    /// <summary>CSS custom-property name for the circular track color token.</summary>
    public const string CircularTrackColor = $"{Prefix}-circular-track-color";
    /// <summary>CSS custom-property name for the linear height token.</summary>
    public const string LinearHeight = $"{Prefix}-linear-height";
    /// <summary>CSS custom-property name for the linear height sm token.</summary>
    public const string LinearHeightSm = $"{Prefix}-linear-height-sm";
    /// <summary>CSS custom-property name for the linear height lg token.</summary>
    public const string LinearHeightLg = $"{Prefix}-linear-height-lg";
    /// <summary>CSS custom-property name for the linear radius token.</summary>
    public const string LinearRadius = $"{Prefix}-linear-radius";
    /// <summary>CSS custom-property name for the circular size token.</summary>
    public const string CircularSize = $"{Prefix}-circular-size";
    /// <summary>CSS custom-property name for the circular size sm token.</summary>
    public const string CircularSizeSm = $"{Prefix}-circular-size-sm";
    /// <summary>CSS custom-property name for the circular size lg token.</summary>
    public const string CircularSizeLg = $"{Prefix}-circular-size-lg";
    /// <summary>CSS custom-property name for the circular stroke width token.</summary>
    public const string CircularStrokeWidth = $"{Prefix}-circular-stroke-width";
    /// <summary>CSS custom-property name for the circular stroke width sm token.</summary>
    public const string CircularStrokeWidthSm = $"{Prefix}-circular-stroke-width-sm";
    /// <summary>CSS custom-property name for the circular stroke width lg token.</summary>
    public const string CircularStrokeWidthLg = $"{Prefix}-circular-stroke-width-lg";
    /// <summary>CSS custom-property name for the indeterminate duration token.</summary>
    public const string IndeterminateDuration = $"{Prefix}-indeterminate-duration";
    /// <summary>CSS custom-property name for the indeterminate easing token.</summary>
    public const string IndeterminateEasing = $"{Prefix}-indeterminate-easing";
    /// <summary>CSS custom-property name for the buffer color token.</summary>
    public const string BufferColor = $"{Prefix}-buffer-color";
    /// <summary>CSS custom-property name for the wavy duration token.</summary>
    public const string WavyDuration = $"{Prefix}-wavy-duration";
}

/// <summary>CSS variable tokens for switch field.</summary>
public static class SwitchField
{
    private const string Prefix = $"{Vars.Flare}-switch";

    /// <summary>CSS custom-property name for the track width token.</summary>
    public const string TrackWidth = $"{Prefix}-track-width";
    /// <summary>CSS custom-property name for the track height token.</summary>
    public const string TrackHeight = $"{Prefix}-track-height";
    /// <summary>CSS custom-property name for the track width sm token.</summary>
    public const string TrackWidthSm = $"{Prefix}-track-width-sm";
    /// <summary>CSS custom-property name for the track height sm token.</summary>
    public const string TrackHeightSm = $"{Prefix}-track-height-sm";
    /// <summary>CSS custom-property name for the track width lg token.</summary>
    public const string TrackWidthLg = $"{Prefix}-track-width-lg";
    /// <summary>CSS custom-property name for the track height lg token.</summary>
    public const string TrackHeightLg = $"{Prefix}-track-height-lg";
    /// <summary>CSS custom-property name for the track radius token.</summary>
    public const string TrackRadius = $"{Prefix}-track-radius";
    /// <summary>CSS custom-property name for the track color token.</summary>
    public const string TrackColor = $"{Prefix}-track-color";
    /// <summary>CSS custom-property name for the track border color token.</summary>
    public const string TrackBorderColor = $"{Prefix}-track-border-color";
    /// <summary>CSS custom-property name for the track border width token.</summary>
    public const string TrackBorderWidth = $"{Prefix}-track-border-width";
    /// <summary>CSS custom-property name for the track color selected token.</summary>
    public const string TrackColorSelected = $"{Prefix}-track-color-selected";
    /// <summary>CSS custom-property name for the track border color selected token.</summary>
    public const string TrackBorderColorSelected = $"{Prefix}-track-border-color-selected";
    /// <summary>CSS custom-property name for the thumb size token.</summary>
    public const string ThumbSize = $"{Prefix}-thumb-size";
    /// <summary>CSS custom-property name for the thumb size sm token.</summary>
    public const string ThumbSizeSm = $"{Prefix}-thumb-size-sm";
    /// <summary>CSS custom-property name for the thumb size lg token.</summary>
    public const string ThumbSizeLg = $"{Prefix}-thumb-size-lg";
    /// <summary>CSS custom-property name for the thumb color token.</summary>
    public const string ThumbColor = $"{Prefix}-thumb-color";
    /// <summary>CSS custom-property name for the thumb color selected token.</summary>
    public const string ThumbColorSelected = $"{Prefix}-thumb-color-selected";
    /// <summary>CSS custom-property name for the thumb icon color token.</summary>
    public const string ThumbIconColor = $"{Prefix}-thumb-icon-color";
    /// <summary>CSS custom-property name for the thumb icon color selected token.</summary>
    public const string ThumbIconColorSelected = $"{Prefix}-thumb-icon-color-selected";
    /// <summary>CSS custom-property name for the thumb shadow token.</summary>
    public const string ThumbShadow = $"{Prefix}-thumb-shadow";
    /// <summary>CSS custom-property name for the focus outline width token.</summary>
    public const string FocusOutlineWidth = $"{Prefix}-focus-outline-width";
    /// <summary>CSS custom-property name for the focus outline color token.</summary>
    public const string FocusOutlineColor = $"{Prefix}-focus-outline-color";
    /// <summary>CSS custom-property name for the focus outline offset token.</summary>
    public const string FocusOutlineOffset = $"{Prefix}-focus-outline-offset";
    /// <summary>CSS custom-property name for the transition duration token.</summary>
    public const string TransitionDuration = $"{Prefix}-transition-duration";
    /// <summary>CSS custom-property name for the transition easing token.</summary>
    public const string TransitionEasing = $"{Prefix}-transition-easing";
    /// <summary>CSS custom-property name for the pressed layer color token.</summary>
    public const string PressedLayerColor = $"{Prefix}-pressed-layer-color";
    /// <summary>CSS custom-property name for the pressed layer opacity token.</summary>
    public const string PressedLayerOpacity = $"{Prefix}-pressed-layer-opacity";
    /// <summary>CSS custom-property name for the disabled opacity token.</summary>
    public const string DisabledOpacity = $"{Prefix}-disabled-opacity";
}
