...
Тема Material Design 2, светлая и тёмная, для библиотеки компонентов
...
| Карточки | `FlareCard` | M2 рисует одну карточку; `Variant` всё равно предлагает три из M3 |
...
- **Иерархию несёт тень**, а не тональный цвет поверхности: M2 опирается на тени там, где M3 опирается
