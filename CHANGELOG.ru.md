...
- **`FlareSlider`, `FlarePagination` и `FlareRating` потеряли геометрию в теме Material Design 3**
  ...
  `--flare-slider-track-height-xs` … `-xl` (аналогично радиус трека, высота ручки, `--flare-rating-size-*` и
  `--flare-pagination-size-*`), ровно та схема, которую `FlareButton` уже использует для своих пер-размерных
  отступов и высот. Тема эмитит все пять, а size-класс компонента лишь выбирает, какой читать: рампа живёт в
  теме, CSS компонента не содержит значений геометрии вообще, а тема теперь может задать настоящую рампу
  вместо одного плоского значения. Отрисовка во всех темах не изменилась.
