...
- **`FlareSlider`, `FlarePagination` и `FlareRating` потеряли геометрию в теме Material Design 3**
  ...
  Тема ставит геометрический токен в `initial`, что означает «я это не переопределяю - используй
  собственный пер-размерный дефолт компонента», и `var(--token, <fallback>)` тогда намеренно пропускает его
  и берёт фолбэк, потому что `initial` - это guaranteed-invalid значение для custom property. Проход 0.2.0,
  вычищавший «мёртвые» литеральные фолбэки, снёс именно их - исходя из посылки, что у токена, который
  эмитит каждая тема, фолбэк недостижим. Для «запаркованного» токена это неверно. Без фолбэка подстановка
  даёт пустоту, объявление становится невалидным на этапе вычисления значения, и геометрия молча исчезает.
  Фолбэки восстановлены (идентично значениям до вычистки), а резолвер теперь документирует, почему их
  нельзя трогать.
- **Контракт закреплён guard-тестом**: новый тест падает, если токен, который любая тема паркует в
  `initial`, читается в CSS ядра без фолбэка. Аудит на уровне имён такой класс багов не видит - нужна
  именно пара «запаркованное значение» + «нет фолбэка», из-за чего баг и уехал в три релиза.
