...
### Изменено
- **Разработка тем**: у `InputTokens` появилось обязательное поле `FocusRing` (box-shadow кольца фокуса
  поля, CSS `--flare-input-focus-ring`). Кастомные темы, создающие `InputTokens` напрямую, обязаны его
  задать; темы, наследующие встроенные через `with`, получают его автоматически.
...
- **Восстановлен индикатор фокуса поля**: у всех контролов на базе `FlareField` (`FlareField`,
  ...
  фокуса ни мышью, ни клавиатурой. Теперь на `:focus-within` рисуется layout-neutral кольцо, управляемое
  токеном темы `--flare-input-focus-ring` (варианты filled/outlined его переопределяют). Невалидные поля
  получают кольцо цвета ошибки.
