...
- **`FlareSlider`, `FlarePagination` и `FlareRating` потеряли геометрию в теме Material Design 3**
  ...
  изменилась.
- **Зоны `FlareSlider` срезали скругление трека и слипались.** Зона рисовалась голым прямоугольником: без
  радиуса и без отступа у насечки, поэтому зона, доходящая до конца трека, закрывала скруглённый угол рельса
  (трек выглядел квадратным везде, где зоны доходят до края - то есть всегда у шкалы во весь диапазон), а
  соседние зоны стыковались вплотную без разделения. Зона - это полоса на том же рельсе, что и
  активный/неактивный сегменты, поэтому теперь она говорит на том же языке формы и из тех же токенов темы:
  край на внешнем конце трека сохраняет полный радиус трека, внутренний край получает радиус зазора и
  отступ на величину насечки. В итоге зоны разделены в темах, где зазор задан (Material Design 3 /
  Expressive теперь дают то же разделение 2x`--flare-slider-gap`, что и разрез активного/неактивного), и
  остаются встык там, где зазор равен 0 (FluentUI2, Visual Studio) - без per-theme CSS.
