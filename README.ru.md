...
дашборд, административный грид, мастер, редактор с разделителем, - знать HTML и CSS не требуется: всё
...
компоненте: первая - обещание, вторая - то, что не даёт ему стать клеткой. Всё, где первая половина не
...
провайдер подставляется в любой параметр типа `FlareIcon` (или рисуется отдельно через `<FlareIconView>`). Всё
