...
- **Breaking for custom themes: `LayoutTokens` requires the drawer toggle's geometry.** `AppBarToggleSize`,
  ...
  glyph in every theme. Material Design 3 and 3 Expressive now draw it as their 40px icon button with an 18x12
  glyph, Material Design 2 as a 48px one, and Fluent 2 as its 32px Hamburger with 1px bars and 4px corners;
  Aero, Liquid Glass and Visual Studio keep the 36px round burger.
...
- **Breaking for applications that relied on the boot script's default theme.** `flare-bootstrap.js`
  ...
  worse than not guessing. It now applies only what is saved, or what you name yourself:
  ...
  Set those two to the theme and palette you register in `Program.cs`. Without them the first frame
  is unthemed until the application boots - correct, but bare.
- **The theme engine's interop module is fetched while the runtime loads.** Every Flare application
  needs `flare-theme.js`, but nothing could ask for it until the whole .NET runtime had finished
  loading, so applying the theme waited on a round trip that had no reason to be late. The boot
  script now preloads it, which takes that round trip off the path to the first themed frame.
...
- **The shipped stylesheet carries no comments.** They are written for whoever edits the source,
...
### Toolchain
- **The API reference is regenerated by the release path, not by every build.** `pages.yml` now re-runs
  `Flare.ApiDocGen` before publishing the gallery, so the deployed reference is built from the sources
  being deployed rather than from whatever was last committed; `release.yml` re-runs it on a tag and
  refuses the release if the committed file disagrees with its own sources, which is the failure this
  release was itself an example of. It stays out of the ordinary build on purpose: the generator
  reflects over the built assemblies, so wiring it in would put an inner build on every Debug F5 to keep
  pages current that only the deployed site shows. Locally it is still one flag:
  `dotnet build samples/Flare.Gallery/Flare.Gallery.csproj -p:RunApiDocGen=true`.
- **`llms.txt` described the API reference as a docfx site.** docfx was replaced by the gallery two
  releases ago; the URL survived deliberately, the description did not.
