...
- **A polymorphic icon system: any icon provider fits any icon slot.** `FlareIcon` is an abstract icon
  ...
  - `Flare.Icons.MaterialDesign3.Symbols` - `FlareMaterialIcon` (Material Symbols variable font:
    `Fill`/`Weight`/`Grade`/`OpticalSize`/`Family`).
  - `Flare.Icons.MaterialDesign2.Symbols` - `FlareMaterialClassicIcon` (classic Material Icons webfont:
    Filled/Outlined/Round/Sharp/TwoTone).
  - `Flare.Icons.MaterialDesign2.Svg` - the full Material Icons (filled) set as inline SVG (`MaterialIcons`
    catalog, 2122 icons), generated by `tools/MaterialIconGen`.
  - `Flare.Icons.FluentUI.Svg` - the full Fluent UI System Icons set as inline SVG (`FluentIcons` catalog,
    Regular + Filled, Size 24, ~5000 icons), generated by `tools/FluentIconGen`; plus `FlareFluentIcon`.
...
- **A built-in, dependency-free SVG icon set (`FlareIcons`).** 84 ready `FlareSvgIcon` members (`Home`,
  ...
  icon otherwise - core never falls back to a third-party font. (Removing the string convenience for typed
  icons everywhere is a tracked follow-up: `docs/issues/icon-string-to-typed-migration.md`.)
...
- **BREAKING: `FlareIconButton.Icon` is now a `FlareIcon`, not a Material-name string**, so any provider
  works in an icon button. A bare string resolves to the built-in SVG when known (else empty - no font); a
  Razor literal must be an expression: `Icon="settings"` -> `Icon="@("settings")"` (or
  `Icon="@FlareIcons.Settings"`).
...
- **BREAKING: icon parameters across components become `FlareIcon?` (any provider).** `FlareMenuItem`,
  ...
  string resolves to the built-in SVG when known (else empty - no font); a Razor literal must be an
